  ...
  public partial class EmailTemplate : System.Web.UI.Page
  {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      if (!IsPostBack)
      {
        ...
        liMailtemplate.Attributes.Add("class", "nav-item menu-open");
        aMailtemplate.Attributes.Add("class", "nav-link active");
      }
    ...
    {
      string summernoteContent = hdnSummernote.Value;
      ...
      if (drpEmailFor.SelectedItem.Text == "Select")
      {
        ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "errorsalert('Please select Email For');", true);
        return;
      }
      if (drpEmailStage.SelectedItem.Text == "Select")
      {
        ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "errorsalert('Please select Email Stage');", true);
        return;
      }
      if (txtEmailSubject.Text == "Select")
      {
        ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "errorsalert('Please enter Email Subject');", true);
        return;
      }
      if (string.IsNullOrEmpty(summernoteContent))
      {
        ScriptManager.RegisterStartupScript(this, GetType(), "Popup", "errorsalert('Please enter Email Body');", true);
        return;
      }
      if (Convert.ToInt32(hdnID.Value) > 0)
      {
        string sSqlUpdate = "UPDATE Tbl_EmailTemplate SET EmailType='" + drpEmailType.SelectedItem.Text + "', Email_For='" + drpEmailFor.SelectedItem.Text + "', EmailStage='" + drpEmailStage.SelectedItem.Text + "', Email_Body=N'" + summernoteContent + "', Email_Sub='" + txtEmailSubject.Text + "' WHERE id='" + hdnID.Value + "'";
        ocon.Execute(sSqlUpdate);

        string sDestURL = string.Format("\"{0}\"", "EmailTemplate.aspx?");
        string smessage = string.Format("\"{0}\"", "Successfully Updated");
      ...
      {
        string sSqInsert = " insert into Tbl_EmailTemplate (EmailType,Email_For,Email_Body,Email_Sub,EmailStage) values ('" + drpEmailType.SelectedItem.Text + "','" + drpEmailFor.SelectedItem.Text + "',N'" + summernoteContent + "','" + txtEmailSubject.Text + "','" + drpEmailStage.SelectedItem.Text + "')";
        ocon.Execute(sSqInsert);

        string sDestURL = string.Format("\"{0}\"", "EmailTemplate.aspx?");
        string smessage = string.Format("\"{0}\"", "Successfully Added");
    ...
    {
      if (drpEmailType.SelectedItem.Text != "Select" && drpEmailFor.SelectedItem.Text != "Select" && drpEmailStage.SelectedItem.Text != "Select")
      {
        string sSqlChck = "SELECT * FROM Tbl_EmailTemplate WHERE EmailType='" + drpEmailType.SelectedItem.Text + "' and Email_For='" + drpEmailFor.SelectedItem.Text + "' and EmailStage='" + drpEmailStage.SelectedItem.Text + "'";
        DataTable dt = ocon.GetTable(sSqlChck, new DataSet());

        if (dt.Rows.Count > 0)
        {
          string result = dt.Rows[0]["Email_Body"].ToString();
          hdnSummernote.Value = result ?? string.Empty;

          hdnID.Value = dt.Rows[0]["ID"].ToString();
          txtEmailSubject.Text = dt.Rows[0]["Email_Sub"].ToString();
        }
        else
        {
          hdnSummernote.Value = "";
      ...
      ScriptManager.RegisterStartupScript(this, GetType(), "populateSummernote", "populateSummernote();", true);
    }

 

    protected void drpEmailStage_SelectedIndexChanged(object sender, EventArgs e)
    {
      LoadData();
    }

    protected void drpEmailFor_SelectedIndexChanged(object sender, EventArgs e)
    {
      LoadData();
